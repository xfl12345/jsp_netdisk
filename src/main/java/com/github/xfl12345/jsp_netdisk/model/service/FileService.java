...
import com.github.xfl12345.jsp_netdisk.model.pojo.api.request.payload.ChangeNameOperationData;
import com.github.xfl12345.jsp_netdisk.model.pojo.api.request.payload.DirectoryOperationRequestData;
import com.github.xfl12345.jsp_netdisk.model.pojo.api.request.payload.FileViewListItem;
...
//    }
    ...
    public boolean isAuthorized(HttpSession session, TbDirectory directory, String operation) {
        ...
        if (directory == null ||
                directory.getDirectoryId() == null ||
                directory.getAccountId() == null ||
                directory.getDirectoryName() == null) {
            return false;
        }
        //获取用户信息
    ...
    public boolean isAuthorized(HttpSession session, DirFile dirFile, String operation) {
        ...
        if (dirFile == null || ! dirFile.isAllFieldNotNull()) {
            return false;
        }
        //获取用户信息
        TbAccount tbAccount = (TbAccount) session.getAttribute(MySessionAttributes.TB_ACCOUNT);
                ...
                case jsonApiVersion:
                    ...
                    //删除
                        ...
                        for (UserFile currFile : fileDirectoryView.files.values()) {
                            DirFile currDirFile = getDirFileFromUserFile(currFile);
                            //优先判断有无权限执行文件操作
                    ...
                    //修改名称
                        ...
                        changeNameOperation(responseObject, baseRequestObject, session);
                        break;
                    }
                    //复制
    ...
    public void changeNameOperation(JsonCommonApiResponseObject responseObject,
                                    BaseRequestObject baseRequestObject,
                                    HttpSession session) {
        ChangeNameOperationData data = null;
        TbDirectory directory = null;
        DirFile dirFile = null;
        String destName = null;
        boolean itemIsDir = false;
        //优先检查数据
        try {
            data = baseRequestObject.data.toJavaObject(ChangeNameOperationData.class);
            //优先判断用户自定义名称是否合法
            if( !MyStrIsOK.isValidFileName( data.destName )){
                setJsonApiResult(responseObject, FileApiResult.FAILED_NAME_ILLEGAL);
                return;
            }
            destName = data.destName;
            if(data.item.getItemIsDir() == null)
                throw new Exception("数据不完整，无法判断该项目是文件还是目录");
            itemIsDir = data.item.getItemIsDir();
            //从数据库里拉取数据，并完成简单判断（包括是否存在，是否唯一）
            //如果是目录
            if( itemIsDir ){
                directory = data.item.toTbDirectory(tbDirectoryService);
                //优先判断有无权限执行文件操作
                if (!isAuthorized(session, directory, FileOperationField.changeName)) {
                    setJsonApiResult(responseObject, FileApiResult.FAILED_PERMISSION_DENIED);
                    return;
                }
            }
            //如果是文件
            else {
                dirFile = data.item.toDirFile(dirFileService);
                //优先判断有无权限执行文件操作
                if (!isAuthorized(session, dirFile, FileOperationField.changeName)) {
                    setJsonApiResult(responseObject, FileApiResult.FAILED_PERMISSION_DENIED);
                    return;
                }
            }
        ...
        //如果改名的是目录
        if( itemIsDir ){
            //新建一个空白的 TbDirectory 对象
            TbDirectory destDir = new TbDirectory();
            //浅拷贝请求数据里的源对象
            //（因为被拷贝的对象里面只有基本的数据类型、基本数据类型的包装类，不会拷贝失败）
            TbDirectory checkTbDirectory = destDir.clone();

            checkTbDirectory.setDirectoryName( destName );
            long count = tbDirectoryService.countAll(checkTbDirectory);
            //如果文件名重复了
            if( count != 0 ){
                setJsonApiResult(responseObject, FileApiResult.FAILED_DUPLICATE_NAME);
                return;
            }
            //确保只能修改到一列数据
            destDir.setDirectoryId(directory.getDirectoryId());
            destDir.setDirectoryName( destName );

            //高并发环境下可能存在重名问题
            try {
                affectedRowCount = tbDirectoryService.update(destDir);
                ...
                if (cause instanceof MySQLIntegrityConstraintViolationException) {
                    ...
                    setJsonApiResult(responseObject, FileApiResult.OTHER_FAILED);
                }
        ...
        //如果改名的是文件
            ...
            //新建一个空白的 DirFile 对象
            DirFile destDirFile = new DirFile();
            //浅拷贝请求数据里的源对象
            DirFile checkDestDirFile = dirFile.clone();
            checkDestDirFile.setUserCustomFileName( destName );
            long count = dirFileService.countAll(checkDestDirFile);
            //如果文件名重复了
            if( count != 0 ){
                setJsonApiResult(responseObject, FileApiResult.FAILED_DUPLICATE_NAME);
                return;
            ...
            //确保只能修改到一列数据
            destDirFile.setUserCustomFileName( destName );

            //高并发环境下可能存在重名问题
            try {
                affectedRowCount = dirFileService.update(dirFile, destDirFile);
            } catch (DataAccessException e) {
                Throwable cause = e.getCause();
                //不巧，确实发生了碰撞
                if (cause instanceof MySQLIntegrityConstraintViolationException) {
        ...
        //当且仅当唯一确定地只修改到一行数据，判为操作成功
        if( affectedRowCount == 1){
            setJsonApiResult(responseObject, FileApiResult.SUCCEED);
            return;
        }
        else if( affectedRowCount > 1){
            logger.error("用户ID=" + tbAccount.getAccountId() +"，执行更新文件关联数据居然修改到不止一行数据！！请马上停机检查！");
            //不能给SQL注入攻击者提供操作有效的信号反馈
            setJsonApiResult(responseObject, FileApiResult.FAILED);
        }
        else {
            setJsonApiResult(responseObject, FileApiResult.FAILED);
    ...
    public boolean writeFile(MultipartFile uploadFile, String filePath) {
        ...
        try {
            ...
            bufferedOutputStream = new BufferedOutputStream(outputStream, (1 << 20));//1MiB 内存空间做缓冲
            byte[] tmpReadBuffer = new byte[1024];
            int currReadByteCount = 0;
            while ((currReadByteCount = inputStream.read(tmpReadBuffer, 0, tmpReadBuffer.length)) != -1) {
    ...
    public void download(JsonCommonApiResponseObject responseObject,
        ...
        //检查数据完整
        if (data == null || ! data.isAllFieldNotNull()) {
            setJsonApiResult(responseObject, FileApiResult.FAILED_REQUEST_FORMAT_ERROR);
            return;
        }
        //检查是否记录是否存在（过滤非法请求）
    ...
    public DirFile getDirFileFromUserFile(UserFile userFile) {
        DirFile dirFile = new DirFile();
